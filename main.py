...
with open(os.path.join(os.path.dirname(__file__), 'classes.txt')) as f:
    for line in f:
...
def main(opt,output_path):
    output_name = opt.output
    output_json = os.path.join(output_path,'annotations',opt.output)
    print("Start!")
...
if __name__ == "__main__":
    ...
    output_name = f'{os.path.basename(options.path)}_{date_month_year_hour}'
    output_path = os.path.join(options.path,'..',f"{output_name}_COCO")
    # os.makedirs(output_path,exist_ok=True)

    shutil.copytree(options.path,output_path)
    os.makedirs(os.path.join(output_path,'annotations'),exist_ok=True)

    for directory in ('train','test','val'):
        options.output = f'instances_{directory}2017.json'
        main(options,output_path)

    for directory in os.listdir(output_path):
        if directory in ('train','test','val'):
            prev_name = os.path.join(output_path,directory)
            new_name = os.path.join(output_path,f"{directory}2017")
